...
impl<T: ?Sized> ListSlice<T> {
    ...
    pub unsafe fn head_unsafe(&self, n: usize) -> Option<&T> {
        let data = slice::from_raw_parts(self.chunks, self.chunks_len);
        let len = usize::from_ne_bytes(
            data.get(0..mem::size_of::<usize>())
                .and_then(|x| TryFrom::try_from(x).ok())?,
        );
...
impl<T: ?Sized> core::fmt::Debug for ListSlice<T> {
...
impl<T: ?Sized> core::fmt::Debug for List<T> {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        f.write_str("List(&[ ")?;
...
unsafe impl<T: Copy> CopyableUnsized for [T] {
    unsafe fn construct_ptr<'a>(data: *const u8, len: usize) -> &'a Self {
        debug_assert_eq!(len % mem::size_of::<T>(), 0);
        slice::from_raw_parts(data as *const T, len / mem::size_of::<T>())
    }
}
unsafe impl CopyableUnsized for str {
    unsafe fn construct_ptr<'a>(data: *const u8, len: usize) -> &'a Self {
        core::str::from_utf8_unchecked(slice::from_raw_parts(data, len))
    }
}
...
unsafe impl CopyableUnsized for OsStr {
    unsafe fn construct_ptr<'a>(data: *const u8, len: usize) -> &'a Self {
        OsStr::from_bytes(slice::from_raw_parts(data, len))
    }
}
...
unsafe impl CopyableUnsized for Path {
    unsafe fn construct_ptr<'a>(data: *const u8, len: usize) -> &'a Self {
        Path::new(OsStr::from_bytes(slice::from_raw_parts(data, len)))
    }
}

unsafe impl CopyableUnsized for CStr {
    unsafe fn construct_ptr<'a>(data: *const u8, len: usize) -> &'a Self {
        CStr::from_bytes_with_nul_unchecked(slice::from_raw_parts(data, len))
    }
}
...
impl<T: ?Sized> List<T> {
    ...
    pub fn push(&mut self, data: &T) {
        ...
        let usize_len = mem::size_of::<usize>().max(align);
        ...
        if old_align != self.inner.align || total_len + aligned_len > self.chunks_cap {
            if self.inner.chunks.is_null() {
                self.inner.chunks = unsafe {
                    alloc2::alloc(
                        alloc2::Layout::from_size_align(total_len * 2, self.inner.align).unwrap(),
                    )
                };
                ...
                unsafe {
                    ...
                    self.inner.chunks = alloc2::alloc(
                        alloc2::Layout::from_size_align(
                            (aligned_len + total_len) * 2,
                            self.inner.align,
                        )
                        .unwrap(),
                    ...
                    alloc2::dealloc(
                        old_chunks,
                        alloc2::Layout::from_size_align(self.chunks_cap, old_align).unwrap(),
                    );
                    self.chunks_cap = (aligned_len + total_len) * 2;
                }
            }
